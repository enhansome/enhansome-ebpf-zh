...
BPF，即伯克利数据包过滤器（Berkeley Packet Filter），是一个运行从用户空间传递的程序的内核虚拟机。BPF 最初在 BSD 上实现，随后被移植到 Linux，在内核中运行"经典 BPF"或 cBPF 虚拟机，被 tcpdump 等工具用于过滤数据包，以避免将数据复制到用户空间。最近，Linux 中的 BPF 基础设施已经完全重构，产生了"扩展 BPF"（eBPF），它获得了新功能（程序安全性和终止检查、JIT 编译、持久化映射、标准库、硬件卸载支持等），现在可用于多种任务。在极低层级（XDP）处理数据包、跟踪和监控系统事件、为 cgroups 实施访问控制等，都是 eBPF 提供高性能、可编程性和灵活性的应用实例。

最近，[Cilium](https://cilium.io) 推出了一个关于 eBPF 的优秀网站 [ebpf.io](https://ebpf.io/)。它与本列表有相似的作用，提供了 [eBPF 简介](https://ebpf.io/what-is-ebpf) 以及相关项目的链接。

> 注意：eBPF 是一项令人兴奋的技术，其生态系统正在快速发展。我们非常欢迎您的帮助，以保持这个列表的时效性，并以任何可能的方式提高其信噪比。请随时在[这里](https://github.com/zoidbergwill/awesome-ebpf/issues) ⭐ 5,173 | 🐛 16 | 📅 2026-09-14留下您的反馈。
...
* [ebpf.io](https://ebpf.io/) - eBPF 基础知识的门户网站，包括主要相关项目和社区资源列表。
* [Cilium's BPF and XDP Reference Guide](http://docs.cilium.io/en/latest/bpf/) - 关于 eBPF 大多数特性和方面的深入文档。

### 内核文档

* [BPF 文档](https://www.kernel.org/doc/html/latest/bpf/index.html) - Linux 内核 BPF 相关文档的索引。
* [linux/Documentation/networking/filter.rst](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/Documentation/networking/filter.rst) - eBPF 规范（略有过时；信息仍然有效，但不全面）。
* [BPF 设计问答](https://www.kernel.org/doc/html/latest/bpf/bpf_design_QA.html) - 关于 BPF 基础架构设计决策的常见问题。
* [如何与 BPF 子系统交互](https://www.kernel.org/doc/html/latest/bpf/bpf_devel_QA.html) - 关于 eBPF 开发贡献的常见问题。
...
* [内核版本中的 BPF 特性列表](https://github.com/iovisor/bcc/blob/master/docs/kernel-versions.md) ⭐ 22,673 | 🐛 1,078 | 🌐 C | 📅 2026-09-19
...
* [BPF 参考指南](https://github.com/iovisor/bcc/blob/master/docs/reference_guide.md) ⭐ 22,673 | 🐛 1,078 | 🌐 C | 📅 2026-09-19 - 来自 bcc 仓库的 BPF C 和 bcc Python 辅助函数指南。

* [eBPF入门指南](https://github.com/lizrice/ebpf-beginners) ⭐ 1,729 | 🐛 3 | 🌐 Python | 📅 2023-05-08 - 一组现场编码讲座及其附带的代码示例，介绍使用各种库和程序类型进行eBPF编程。

* [XDP 和 eBPF 简介](https://blogs.igalia.com/dpino/2019/01/07/introduction-to-xdp-and-ebpf/) - 一篇通俗易懂的介绍，提供了 eBPF 的背景、历史和工作原理。
...
* [Ferris Ellis 的 eBPF 博客文章](https://ferrisellis.com/tags/ebpf/) - 包含几篇关于 eBPF 的文章：
...
* [使用 BPF 和 XDP 实现内核网络数据路径的可编程化](http://schd.ws/hosted_files/ossna2017/da/BPFandXDP.pdf) - 涵盖 eBPF 和 XDP 所有基础知识的幻灯片（主要针对网络处理）。

* [BSD 数据包过滤器](https://speakerdeck.com/tuxology/the-bsd-packet-filter) - 主要关注跟踪方面。

* [BPF：跟踪及更多](http://www.slideshare.net/brendangregg/bpf-tracing-and-more) - 主要关注跟踪方面。

* [Linux BPF Superpowers](http://www.slideshare.net/brendangregg/linux-bpf-superpowers) - 主要关注跟踪方面，第一部分附带火焰图。
...
* Greg Marsden制作了一些关于eBPF的文档：
  ...
  * [与用户空间通信](https://blogs.oracle.com/linux/notes-on-bpf-3) - BPF如何与用户空间通信 - BPF映射、perf事件、bpf\_trace\_printk。
...
* [eBPF 和 XDP 指南以及最新的更新 (2017)](https://fosdem.org/2017/schedule/event/ebpf_xdp/)
  * [tc的cls\_bpf高级可编程性和最新更新](http://netdevconf.org/1.2/session.html?daniel-borkmann) - 有关eBPF，它在隧道和封装、直接数据包访问等方面的使用等详细信息。
  * [自netdev 1.1以来的cls\_bpf/eBPF更新](http://netdevconf.org/1.2/slides/oct5/07_tcws_daniel_borkmann_2016_tcws.pdf) - 属于[tc研讨会](http://netdevconf.org/1.2/session.html?jamal-tc-workshop)的一部分。
...
* [eXpress Data Path (XDP)](https://github.com/iovisor/bpf-docs/raw/master/Express_Data_Path.pdf) ⭐ 1,017 | 🐛 3 | 📅 2022-09-20 - 关于XDP的第一份演示文稿。

* 所有XDP的细节都在这篇技术论文中：[The eXpress Data Path: Fast Programmable Packet Processing in the Operating System Kernel](https://github.com/tohojo/xdp-paper) ⭐ 154 | 🐛 2 | 🌐 CSS | 📅 2021-01-15，作者为Toke Høiland-Jørgensen、Jesper Dangaard Brouer、Daniel Borkmann、John Fastabend、Tom Herbert、David Ahern和David Miller，他们都是重要的eBPF和XDP贡献者。
...
* [AF\_XDP](https://www.kernel.org/doc/html/latest/networking/af_xdp.html) - AF\_XDP地址族的内核文档。
* [在Linux中使用AF\_XDP实现快速数据包处理](https://archive.fosdem.org/2018/schedule/event/af_xdp/)。
...
* [hBPF - eBPF 在硬件中的实现](https://github.com/rprinz08/hBPF) ⭐ 435 | 🐛 0 | 🌐 Python | 📅 2023-01-27 - 针对 FPGAs 编写的 eBPF CPU。
* [OpenCSD eBPF SSD 卸载](https://github.com/Dantali0n/qemu-csd) ⭐ 69 | 🐛 0 | 🌐 C++ | 📅 2023-11-01 - 计算存储模拟（QEMU）平台，使用 FUSE LFS 文件系统和 uBPF 进行计算内核卸载，适用于分区命名空间 NVMe SSD，全部在用户空间中。
...
* [bcc 参考指南](https://github.com/iovisor/bcc/blob/master/docs/reference_guide.md) ⭐ 22,673 | 🐛 1,078 | 🌐 C | 📅 2026-09-19 - 许多增量步骤来开始使用 bcc 和 eBPF，大多集中在跟踪和监视上。
* [bcc Python 开发者教程](https://github.com/iovisor/bcc/blob/master/docs/tutorial_bcc_python_developer.md) ⭐ 22,673 | 🐛 1,078 | 🌐 C | 📅 2026-09-19 - 随 bcc 一起提供，但针对 Python 代码，共包含十七个“课程”。
* [XDP 实践教程](https://github.com/xdp-project/xdp-tutorial) ⭐ 2,980 | 🐛 170 | 🌐 C | 📅 2026-05-08 - 渐进式（包含三个难度级别）的教程，学习如何使用 XDP 处理数据包。
...
* [bcc/examples](https://github.com/iovisor/bcc/tree/master/examples) ⭐ 22,673 | 🐛 1,078 | 🌐 C | 📅 2026-09-19 - 与 bcc 工具一起提供的示例，主要与跟踪有关。
* [bcc/tools](https://github.com/iovisor/bcc/tree/master/tools) ⭐ 22,673 | 🐛 1,078 | 🌐 C | 📅 2026-09-19 - 这些工具本身可以被视为 BPF 程序的示例用例，主要用于追踪和监视。 bcc 工具已针对一些 Linux 发行版进行了打包。
...
* [Netronome sample network applications](https://github.com/Netronome/bpf-samples/) ⭐ 109 | 🐛 6 | 🌐 C | 📅 2020-04-06 - 提供基本但完整的 eBPF 应用程序示例，也与硬件卸载兼容。
* [ebpf-samples](https://github.com/vbpf/ebpf-samples) ⭐ 94 | 🐛 0 | 🌐 C | 📅 2026-07-03 - 多个项目中收集的编译（作为 ELF 对象文件）的示例集合，主要用作用户空间验证器的测试用例。
* [MPLSinIP sample](https://github.com/fzakaria/eBPF-mpls-encap-decap) ⭐ 57 | 🐛 0 | 🌐 C | 📅 2019-10-18 - 大量注释的示例，演示了如何在 IP 中封装和解封装 MPLS。 该代码适用于 BPF 开发新手。
* [ebpf-kill-example](https://github.com/niclashedam/ebpf-kill-example) ⭐ 24 | 🐛 1 | 🌐 C | 📅 2026-07-13 - 完全记录和测试过的 eBPF 探针示例，可记录所有强制终止并在用户空间中将其打印出来。
...
* [bcc](https://github.com/iovisor/bcc/) ⭐ 22,673 | 🐛 1,078 | 🌐 C | 📅 2026-09-19 - 框架和工具 - 一种处理 BPF 程序的方法，特别是用于跟踪和监控。还包括一些实用程序，可帮助检查系统中的映射或程序。
* [bcc 的 Lua 前端](https://github.com/iovisor/bcc/tree/master/src/lua) ⭐ 22,673 | 🐛 1,078 | 🌐 C | 📅 2026-09-19 - 另一种替代 C，甚至是 bcc 中大多数 Python 代码的选择。
...
* [libbpf](https://git.kernel.org/pub/scm/linux/kernel/git/davem/net-next.git/tree/tools/lib/bpf) - 一个用于处理BPF对象（程序和映射）和操作包含它们的ELF对象文件的C库。它随内核一起提供，并在[GitHub上进行了镜像](https://github.com/libbpf/libbpf) ⭐ 2,758 | 🐛 32 | 🌐 C | 📅 2026-09-15。
* [libbpf-bootstrap](https://github.com/libbpf/libbpf-bootstrap) ⭐ 1,524 | 🐛 18 | 🌐 CMake | 📅 2026-09-08 - 用于使用libbpf和BPF CO-RE进行BPF应用开发的脚手架。

### Go 库

* [cilium/ebpf](https://github.com/cilium/ebpf) ⭐ 7,968 | 🐛 31 | 🌐 Go | 📅 2026-09-23 - 纯Go库，用于读取、修改、加载eBPF程序并将其附加到Linux内核中的各种钩子。
* [gobpf](https://github.com/iovisor/gobpf) ⭐ 2,179 | 🐛 82 | 🌐 C | 📅 2023-08-31 - 用于创建eBPF程序的BCC的Go绑定。
* [libbpfgo](https://github.com/aquasecurity/libbpfgo) ⭐ 850 | 🐛 35 | 🌐 Go | 📅 2026-09-22 - 由libbpf支持的用于Go的eBPF库。

### Aya

* [aya](https://github.com/aya-rs/aya) ⭐ 4,822 | 🐛 204 | 🌐 Rust | 📅 2026-09-24 - 一种用纯 Rust 编写、加载和管理 eBPF 对象的库，专注于开发人员体验和可操作性。它支持在 Rust 中编写 eBPF 程序，并通过 crates.io 分发库代码以在 eBPF 程序之间共享。Aya 不依赖于 libbpf。
* [aya-template](https://github.com/aya-rs/aya-template) ⭐ 143 | 🐛 6 | 🌐 Rust | 📅 2026-09-22 - 用于在 Aya 中编写 BPF 应用程序的模板，可以与 [`cargo generate`](https://github.com/cargo-generate/cargo-generate) ⭐ 2,486 | 🐛 30 | 🌐 Rust | 📅 2026-09-24 一起使用。

### zbpf

* [zbpf](https://github.com/tw4452852/zbpf) ⭐ 279 | 🐛 5 | 🌐 Zig | 📅 2026-04-19 - 一个用纯 Zig 编写的跨平台 eBPF 程序框架，由 libbpf 和 Zig 工具链提供支持。

### eunomia-bpf

* [eunomia-bpf](https://github.com/eunomia-bpf/eunomia-bpf) ⭐ 902 | 🐛 15 | 🌐 Rust | 📅 2026-08-09 - 一个编译框架和运行库，可用于构建、分发、动态加载和运行多语言和WebAssembly的CO-RE eBPF应用程序。它支持仅写eBPF内核代码（以构建简单的CO-RE libbpf eBPF应用程序）、同时以BCC和libbpf风格编写内核部分，以及在WASM模块中以多种语言编写用户空间，并使用简单的JSON数据或WASM OCI镜像分发它。运行时仅基于libbpf，并提供CO-RE给BCC-style的eBPF程序，而不依赖于LLVM库。

### oxidebpf

* [oxidebpf](https://github.com/redcanaryco/oxidebpf) ⚠️ Archived - 一个纯Rust库，用于管理eBPF程序，专为安全用例设计。功能集比其他库更有限，但强调在广泛的内核范围和向后兼容的编译一次运行多个地方方面的稳定性。
...
* [rbpf](https://github.com/qmonnet/rbpf) ⭐ 1,130 | 🐛 19 | 🌐 Rust | 📅 2026-06-03 - 用 Rust 编写。用于 Linux、macOS 和 Windows 的解释器，以及在 Linux 下用于 x86\_64 的 JIT 编译器。
* [uBPF](https://github.com/iovisor/ubpf/) ⭐ 989 | 🐛 17 | 🌐 C | 📅 2026-09-24 - 用 C 编写。包含一个解释器、一个用于 x86\_64 架构的 JIT 编译器、一个汇编器和反汇编器。
* [PREVAIL](https://github.com/vbpf/ebpf-verifier) ⭐ 472 | 🐛 39 | 🌐 C++ | 📅 2026-09-19 - 用于 eBPF 的用户空间验证器，[使用抽象解释层实现](https://elazarg.github.io/pldi19main-final.pdf)，支持循环。
...
* [eBPF for Windows](https://github.com/microsoft/ebpf-for-windows) ⭐ 3,564 | 🐛 341 | 🌐 C | 📅 2026-09-24 - 这个项目还在进行中，它允许使用现有的eBPF工具链和Linux生态系统中熟悉的API在Windows上使用。

### 在虚拟环境中进行测试

* [Docker容器中的bcc](https://github.com/zlim/bcc-docker) ⭐ 45 | 🐛 4 | 🌐 Makefile | 📅 2020-07-23。
...
* [Cilium](https://cilium.io/) 项目 ([GitHub 代码库](https://github.com/cilium/cilium) ⭐ 25,533 | 🐛 1,099 | 🌐 Go | 📅 2026-09-24) 是一项依赖于 BPF 和 XDP 技术的项目，可为基于容器的快速内核网络和安全策略实施提供“即时生成的eBPF程序”的支持。有许多演示文稿可用（有些内容重复）：

* [merbridge](https://github.com/merbridge/merbridge/) ⭐ 806 | 🐛 36 | 🌐 Go | 📅 2025-05-19 - 使用eBPF加速您的Service Mesh。Merbridge使用eBPF替换iptables规则以拦截流量。它还结合了msg\_redirect以减少延迟，并在sidecars和服务之间缩短了数据路径。
...
* [P4 on the Edge](https://schd.ws/hosted_files/2016p4workshop/1d/Intel%20Fastabend-P4%20on%20the%20Edge.pdf) - 使用 eBPF 创建高性能可编程交换机的 P4。
  * [P4 使用 eBPF 的旧文档](https://github.com/iovisor/bcc/tree/master/src/cc/frontends/p4) ⭐ 22,673 | 🐛 1,078 | 🌐 C | 📅 2026-09-19 - 来自 bcc 存储库；由下面链接的 P4\_16 后端弃用。
  * [eBPF 的 P4\_16 后端](https://github.com/p4lang/p4c/blob/master/backends/ebpf/README.md) ⭐ 839 | 🐛 483 | 🌐 C++ | 📅 2026-09-24。
...
* [DPDK有一个基于AF\_XDP的轮询模式驱动程序(PMD)](https://dpdkuserspace2018.sched.com/event/G45Z/dpdk-pmd-for-afxdp)
...
* [Suricata文档中“eBPF和XDP”部分](http://suricata.readthedocs.io/en/latest/capture-hardware/ebpf-xdp.html?highlight=XDP#ebpf-and-xdp)
  * [SEPTun-Mark-II](https://github.com/pevma/SEPTun-Mark-II) ⭐ 121 | 🐛 2 | 🌐 Makefile | 📅 2018-04-17 - 极致性能调优指南 - Mark II。
...
* [pixie](https://github.com/pixie-io/pixie) ⭐ 6,539 | 🐛 396 | 🌐 C++ | 📅 2026-09-24 - 使用eBPF进行Kubernetes的可观测性分析。支持协议跟踪，应用程序分析以及分布式bpftrace部署等功能。
* [Hubble](https://github.com/cilium/hubble) ⭐ 4,346 | 🐛 44 | 🌐 Makefile | 📅 2026-09-22 - 使用eBPF为Kubernetes提供网络、服务和安全可观测性。
* [Caretta](https://github.com/groundcover-com/caretta) ⭐ 2,064 | 🐛 18 | 🌐 Go | 📅 2025-03-17 - 通过eBPF生成的即时Kubernetes服务依赖图，可直接输出到Grafana示例。
...
* [parca-agent](https://github.com/parca-dev/parca-agent) ⭐ 746 | 🐛 185 | 🌐 Go | 📅 2026-09-22 - 基于eBPF的持续分析CPU和内存使用情况的分析器，可分析到代码行和时间流逝情况。
* [SkyWalking Rover](https://github.com/apache/skywalking-rover) ⭐ 238 | 🐛 3 | 🌐 Go | 📅 2026-08-28 - [Apache SkyWalking](https://skywalking.apache.org/)是一个专门为分布式微服务、云原生和基于容器（Kubernetes）架构而设计的开源应用程序性能监控（APM）平台。SkyWalking Rover是一个基于eBPF的调试器和度量收集器，支持C、C++、Golang和Rust应用程序。
...
* [Tetragon](https://github.com/cilium/tetragon) ⭐ 5,032 | 🐛 286 | 🌐 C | 📅 2026-09-24 - 面向 Kubernetes、基于 eBPF 的安全监测和运行时强制执行工具。
* [Tracee](https://github.com/aquasecurity/tracee) ⭐ 4,623 | 🐛 121 | 🌐 Go | 📅 2026-09-15 - 一款用于 Linux 的运行时安全和取证工具，它使用 eBPF 技术对系统和应用程序进行跟踪，在运行时分析收集的事件以检测可疑的行为模式。
* [Sysmon for Linux](https://github.com/Sysinternals/SysmonForLinux) ⭐ 2,159 | 🐛 41 | 🌐 C | 📅 2026-09-14 - 一款安全监控工具。它依赖于[SysinternalsEBPF](https://github.com/Sysinternals/SysinternalsEBPF) ⭐ 288 | 🐛 15 | 🌐 C | 📅 2026-09-14。
* [bpflock - 锁定 Linux 机器](https://github.com/linux-lock/bpflock) ⭐ 157 | 🐛 3 | 🌐 C | 📅 2022-02-16 - 一种基于 eBPF 的安全工具，用于锁定和审计 Linux 机器。
* [redcanary-ebpf-sensor](https://github.com/redcanaryco/redcanary-ebpf-sensor) ⭐ 115 | 🐛 0 | 🌐 C | 📅 2026-06-26 - 一组 BPF 程序，从 Linux 内核中收集与安全相关的事件数据。BPF 程序组合成一个单独的 ELF 文件，可以根据运行的操作系统和内核版本选择性加载单个探针。
...
* [inspektor-gadget](https://github.com/inspektor-gadget/inspektor-gadget) ⭐ 2,928 | 🐛 428 | 🌐 C | 📅 2026-09-24 - 基于 eBPF 的工具集合，用于调试和检查 Kubernetes 资源和应用程序。
* [kubectl trace](https://github.com/iovisor/kubectl-trace) ⭐ 2,188 | 🐛 49 | 🌐 Go | 📅 2026-04-16 - 一个用于在 Kubernetes 集群中执行 bpftrace 程序的 kubectl 插件。
* [redbpf](https://github.com/foniod/redbpf) ⚠️ Archived - 用于高效编写 Rust 中的 eBPF 代码的工具和框架。
* [bpfd](https://github.com/genuinetools/bpfd) ⭐ 484 | 🐛 5 | 🌐 Go | 📅 2021-05-07 - 运行带有 Linux 规则的 BPF 程序的框架。 容器感知。
* [adeb](https://github.com/joelagnel/adeb) ⚠️ Archived - 用于在拥有 BPFd 的 Android 上使用跟踪工具的 Linux shell 环境。
* [upf-bpf](https://github.com/navarrothiago/upf-bpf) ⭐ 217 | 🐛 17 | 🌐 C++ | 📅 2024-09-28 - 基于 XDP 的内核解决方案，适用于 5G UPF。
* [BPFd](https://github.com/joelagnel/bpfd) ⚠️ Archived - 明显的 BPF 守护程序，试图利用 bcc 工具的灵活性来跟踪和调试远程目标，特别是在运行 Android 的设备上。
* [greggd](https://github.com/olcf/greggd) ⭐ 6 | 🐛 4 | 🌐 Go | 📅 2025-03-20 - 系统守护进程，用于将 eBPF 程序编译和加载到内核中，并将程序输出转发到套接字以进行度量聚合。
...
* [TripleCross](https://github.com/h3xduck/TripleCross) ⭐ 1,980 | 🐛 19 | 🌐 C | 📅 2024-04-07 - 一个Linux eBPF rootkit，具有后门、C2、库注入、执行劫持、持久性和隐蔽性能力。
* [ebpfkit](https://github.com/Gui774ume/ebpfkit) ⭐ 861 | 🐛 4 | 🌐 C | 📅 2023-02-28 - 利用多种eBPF特性实施攻击性安全技术的rootkit。
* [Bad BPF](https://github.com/pathtofile/bad-bpf) ⭐ 698 | 🐛 2 | 🌐 C | 📅 2024-07-07 - 恶意eBPF程序的集合，利用eBPF在用户模式程序和内核之间读写用户数据的能力。
* [ebpfkit-monitor](https://github.com/Gui774ume/ebpfkit-monitor) ⭐ 145 | 🐛 2 | 🌐 C | 📅 2023-02-28 - 一个实用程序，可在运行时静态分析eBPF字节码或监视可疑的eBPF活动。它专为检测ebpfkit而设计。
...
* [linux/kernel/trace/bpf\_trace.c](https://git.kernel.org/cgit/linux/kernel/git/torvalds/linux.git/tree/kernel/trace/bpf_trace.c) - 与跟踪和监控相关的函数和 eBPF 帮助程序（kprobes，tracepoints 等）。

* JIT 编译器位于其各自体系结构的目录下，例如文件 [linux/arch/x86/net/bpf\_jit\_comp.c](https://git.kernel.org/cgit/linux/kernel/git/torvalds/linux.git/tree/arch/x86/net/bpf_jit_comp.c) 用于 x86. 对于用于硬件卸载的 JIT 编译器，其驱动程序会有相应的文件，例如 [linux/drivers/net/ethernet/netronome/nfp/bpf/jit.c](https://git.kernel.org/cgit/linux/kernel/git/torvalds/linux.git/tree/drivers/net/ethernet/netronome/nfp/bpf/jit.c) 是针对 Netronome NFP 的。
...
* [XDP 协作项目](https://github.com/xdp-project/xdp-project) ⭐ 314 | 🐛 6 | 🌐 CSS | 📅 2026-08-12 - 一个 GitHub 仓库，提供关于 XDP 未来发展的笔记和想法。
...
* [IO Visor 的 bcc 文档](https://github.com/iovisor/bcc/tree/master/docs) ⭐ 22,673 | 🐛 1,078 | 🌐 C | 📅 2026-09-19
...
感谢 Quentin Monnet 和 Daniel Borkmann 对[深入理解 BPF：阅读材料列表](https://qmonnet.github.io/whirl-offload/2016/09/01/dive-into-bpf/)的原创工作，为该列表奠定了基础。
...
在法律允许的最大范围内，zoidbergwill 已放弃对此作品的所有版权和相关权利。
